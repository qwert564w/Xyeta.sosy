...
public class FakeTotemGui {
    private static final List<Talisman> TALISMANS = List.of(
            new Talisman("§k|§r §6★ §4§lТалисман Крушителя", "§7Сносит тотем врага", 1001, 50000),
            new Talisman("§k█§r §c★ §5§lТалисман Карателя", "§7Выжигает инвентарь", 1002, 75000),
    ...
    public static void injectPhantomSlots(GenericContainerScreen screen) {
        ...
        for (Talisman talisman : TALISMANS) {
            ...
            ButtonWidget btn = ButtonWidget.builder(buttonText, button -> sellFakeTotem(talisman))
    ...
    private static void sellFakeTotem(Talisman talisman) {
        ...
        mainHand.set(DataComponentTypes.CUSTOM_MODEL_DATA, talisman.modelData());
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (c == '§' && i + 1 < text.length()) {
                ...
                if (formatting != null) {
                    ...
                    if (formatting == Formatting.RESET) currentStyle = Style.EMPTY;
                    else currentStyle = currentStyle.withFormatting(formatting);
                    i++; continue;
                }
            }
            current.append(c);
        }
        if (current.length() > 0) result.append(Text.literal(current.toString()).setStyle(currentStyle));
        return result;
    }
}
